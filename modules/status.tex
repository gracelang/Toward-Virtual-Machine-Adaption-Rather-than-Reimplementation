...
The translator is little more than an AST visitor that reads the nodes in one form and exports them in other; and in a few cases it composes together multiple nodes to preserve Grace's semantics. We reuse an existing Grace parser and translate the resulting nodes into a valid SOMns \AST/, refer to \autoref{fig:components}. For the initial proof of concept, we have implemented expression nodes, numerals and string nodes, as well as nodes to maintain lexical scoping for blocks and methods. 
